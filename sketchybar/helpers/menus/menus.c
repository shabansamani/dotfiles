...
void ax_perform_click(AXUIElementRef elem) {
  if (!elem)
    return;
  AXUIElementPerformAction(elem, kAXCancelAction);
  usleep(150000);
  AXUIElementPerformAction(elem, kAXPressAction);
}

CFStringRef ax_get_title(AXUIElementRef elem) {
...
extern void SLSSetMenuBarVisibilityOverrideOnDisplay(int cid, int did,
                                                     bool enabled);
extern void SLSSetMenuBarInsetAndAlpha(int cit, double u1, double u2,
                                       float alpha);
