...
static inline void network_update(struct network *net) {
  ...
  u_int64_t ibytes_nm1 = net->data.ifmd_data.ifi_ibytes;
  u_int64_t obytes_nm1 = net->data.ifmd_data.ifi_obytes;
  ifdata(net->row, &net->data);

  double time_scale = (net->tv_delta.tv_sec + 1e6 * net->tv_delta.tv_usec);
